...
- Triage variant of the modal posts to `/_partial/triage/{id}/edit`. Since ADR-012 Amendment 2026-07-26 that endpoint answers with the NEXT item's modal into `#tx-modal-host` and sets `HX-Trigger: queueDirty` (the queue list reconciles once, on modal close); `closeModal` is sent only when the queue is exhausted.
