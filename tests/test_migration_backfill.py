...
def test_run_backfill_dry_run_persists_nothing(
    in_memory_db: sqlite3.Connection, backfill_data_dir: Path
) -> None:
    """``dry_run=True`` runs the full umbrella (parse, validate, pair, etc.)
    against the DB inside a transaction that is rolled back at the end —
    no transactions, no rates, no import_runs, and no earn_positions are
    persisted. Mirrors the BCV reference pattern (see
    ``finances/ingest/bcv.py:155-220``)."""
    from finances.migration.backfill import run_backfill

    report = run_backfill(in_memory_db, backfill_data_dir, dry_run=True)

    txn_count = _count(in_memory_db, "SELECT COUNT(*) FROM transactions")
    assert txn_count == 0, "dry-run must not persist transactions"

    rates_count = _count(in_memory_db, "SELECT COUNT(*) FROM rates")
    assert rates_count == 0, "dry-run must not persist rates"

    runs_count = _count(
        in_memory_db, "SELECT COUNT(*) FROM import_runs WHERE source='backfill'"
    )
    assert runs_count == 0, "dry-run must not persist import_runs umbrella row"

    earn_count = _count(in_memory_db, "SELECT COUNT(*) FROM earn_positions")
    assert earn_count == 0, "dry-run must not persist earn_positions"

    # Report still reflects the would-be inserts (validation + pairing ran).
    assert report.binance_rows_inserted >= 1
    assert report.provincial_rows_inserted >= 1


def test_run_backfill_dry_run_after_real_run_does_not_double_count(
    in_memory_db: sqlite3.Connection, backfill_data_dir: Path
) -> None:
    """A real backfill followed by a ``dry_run=True`` re-run leaves the
    real-run data intact and reports zero would-be inserts (because every
    row is already deduped via ``UNIQUE(source, source_ref)``)."""
    from finances.migration.backfill import run_backfill

    run_backfill(in_memory_db, backfill_data_dir)
    real_total = _count(in_memory_db, "SELECT COUNT(*) FROM transactions")
    real_rates = _count(in_memory_db, "SELECT COUNT(*) FROM rates")
    real_runs = _count(
        in_memory_db, "SELECT COUNT(*) FROM import_runs WHERE source='backfill'"
    )
    assert real_total > 0, "sanity: real run must have inserted rows"

    second = run_backfill(in_memory_db, backfill_data_dir, force=True, dry_run=True)

    # Real-run data untouched.
    after_total = _count(in_memory_db, "SELECT COUNT(*) FROM transactions")
    after_rates = _count(in_memory_db, "SELECT COUNT(*) FROM rates")
    after_runs = _count(
        in_memory_db, "SELECT COUNT(*) FROM import_runs WHERE source='backfill'"
    )
    assert after_total == real_total, "dry-run must not corrupt real-run data"
    assert after_rates == real_rates, "dry-run must not corrupt real-run rates"
    assert after_runs == real_runs, (
        "dry-run must not append an umbrella import_runs row"
    )

    # Idempotent re-run: every source_ref already exists, so zero would-be
    # inserts get reported on the second pass.
    assert second.binance_rows_inserted == 0
    assert second.provincial_rows_inserted == 0
    assert second.bcv_rates_inserted == 0
